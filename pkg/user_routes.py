...
def breakout():
    ...
    if request.method == 'GET':
        ...
        if mytopics:
            #delete previous registration before insert
            ...
            db.session.commit()
            flash("Your registration successful") 
            return redirect('/dashboard')
        else:
...
def donation():
    ...
    if request.method=='GET':
        ...
        if fullname !="" and email !="" and amt !="":
            donate = Donation(donate_donor=fullname, donate_amt=amt, donate_email=email, donate_status='pending', donate_ref=ref)
...
def confirm():
    ...
    if ref:
        ...
        return render_template('user/confirmation.html', deets=deets, donation_deets=donation_deets)
    else:
...
def topaystack():
    ...
    if ref:
        ...
        headers ={"Content-Type": "application/json","Authorization":"Bearer sk_test_ead50eacadfcfa37f1e5f65f95551db34e36513b"}
        trasaction_deets = Donation.query.filter(Donation.donate_ref==ref).first()
        data = {'email': trasaction_deets.donate_email, "amount": trasaction_deets.donate_amt * 100, "reference":ref}
...
@login_required
def paylanding():
    """we will receive Ref number from paystack, call another endpoint to verify the status of the transaction, based on that.... we will update our database"""
    id = session.get('useronline')
